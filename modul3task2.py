...
def get_numbers_ticket(min_value: int, max_value: int, quantity: int) -> list:
    ...
    try:
        ...
        if min_value >= 1 and max_value <= 1000 and quantity <= (max_value - min_value + 1):
            ...
            # повертаємо відсортований список унікальних випадкових чисел
            return sorted(sample(range(min_value, max_value + 1), quantity))
        else:
            # якщо межі діапазону задані невірно → виводимо підказку
            ...
            # якщо quantity виходить за допустимі межі → виводимо підказку
            ...
            # у будь-якому випадку повертаємо порожній список, як вимагає завдання
            return []
    # перехоплюємо помилки типів чи значень
